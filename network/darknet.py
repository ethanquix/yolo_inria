...
def cpu(matrix): # Copy a tensor to gpu if is gpu else no
    if matrix.is_cuda:
...
def create_modules(blocks):
    ...
    for x in blocks:
        # We create a default module
        module = nn.Sequential()

        if (x["type"] == "net"):
            continue

        # If it's a convolutional layer so we create a conv one
        if (x["type"] == "convolutional"):
            # Get the info about the layer and apply it to the activation one
